...
def get_cifar10(split):
    if split == 'train_repr' or split == 'eval_repr':
        ...
        return AugDatasetWrapper(dataset, inv_transform)
...
def get_imagenet(split):
    if split == 'train_repr' or split == 'eval_repr':
        ...
        inv_transform = torchvision.transforms.Compose([
            ...
            torchvision.transforms.RandomGrayscale(p=0.2),
            torchvision.transforms.RandomApply([
                torchvision.transforms.GaussianBlur(kernel_size=tuple(2*round((x - 10)/20) + 1 for x in (224, 224)), sigma=(0.1, 2.0))
            ], p=0.5),
            torchvision.transforms.ToTensor(),
...
class AugDatasetWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, inv_transform):
    ...
    def __getitem__(self, idx):
        ...
        img_1 = self.inv_transform(img)
        img_2 = self.inv_transform(img)
