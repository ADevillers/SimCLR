...
class TrackedValue():
    ...
    def reduce(self):
        old_count = copy.deepcopy(self.count)
        ...
        if abs(new_count - old_count*self.world_size) >= self.world_size:
            print(f'WARNING: A reduction issue may have occurred (abs({new_count} - {old_count}*{self.world_size}) >= {self.world_size}).')
...
class Tracker():
    ...
    def reset(self, name):
        self.tracked_values[name] = TrackedValue(self.device, self.world_size)
